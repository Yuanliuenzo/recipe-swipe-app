...
app.use((req, res, next) => {
  ...
  console.log(
    `${req.method} ${req.url} | ip=${req.ip} xff=${forwardedFor} | ua=${userAgent}`
  );
  next();
});
...
app.use(async (req, res, next) => {
  ...
  if (username) {
    ...
    if (userData && userData.preferences) {
      ...
      console.log(
        "✅ Preferences set in middleware:",
        JSON.stringify(req.user.preferences, null, 2)
      );
    } else {
...
app.post("/set-profile", (req, res) => {
  return res
    .status(410)
    .json({ error: "This endpoint is deprecated. Please use /login instead." });
});
