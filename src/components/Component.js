...
export class Component {
  constructor(container, props = {}) {
    this.container = typeof container === 'string' ? DomUtils.find(container) : container;
    this.props = { ...props };
    this.state = {};
  ...
  render() {
    throw new Error('render() method must be implemented by component subclass');
  }
  ...
  unmount() {
    ...
    // Emit unmount event
    globalEventBus.emit('component:unmounted', { component: this, id: this.id });
  }
  
  // Update component state and re-render
  setState(updates, callback = null) {
    if (!this.isMounted) {
      console.warn(`Component ${this.id}: Cannot setState on unmounted component`);
      return;
    }
  ...
  addEventListener(element, event, handler, options = {}) {
    if (!this.isMounted) {
      console.warn(`Component ${this.id}: Cannot add event listener to unmounted component`);
      return null;
    }
